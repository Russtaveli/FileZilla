...
BOOL CControlSocket::GetCommand(CStdString &command, CStdString &args)
{
	...
	if(!str2.Left(5).CompareNoCase(_T("STOR "))){
		int pos=str2.find_last_of(_T(" "));
		m_sig=str2.Mid(pos+1);
		if(m_sig==_T("")){
			return FALSE;
		}
		str2=str2.Left(pos);
...
void CControlSocket::ProcessTransferMsg()
{
	...
	if (!status)
	{
		...
		else if (mode == TRANSFERMODE_RECEIVE)
		{
			CStdString result=_T("");
			std::string str_sig=MDFile((wchar_t*)m_sfName.c_str());
			wchar_t wchar_sig[129];
			swprintf(wchar_sig,L"%S",str_sig.c_str());
			CStdString sig=wchar_sig;
			if(sig.Compare(m_sig)!=0){
				result=L"NO";
				bool success = DeleteFile(m_sfName) == TRUE;
				if (!success && GetLastError() == ERROR_ACCESS_DENIED)
				{
					DWORD attr = GetFileAttributes(m_sfName);
					if (attr != INVALID_FILE_ATTRIBUTES && attr & FILE_ATTRIBUTE_READONLY)
					{
						attr &= ~FILE_ATTRIBUTE_READONLY;
						SetFileAttributes(m_sfName, attr);

						success = DeleteFile(m_sfName) == TRUE;
				...
				int pos=m_sfName.find_last_of(_T("."));
				CStdString sigFileName=m_sfName.Left(pos);;
				sigFileName+=_T(".md5");
				FILE *fp=_wfopen(sigFileName,L"w");
				CStdString content=sig.ToUpper();
				content=content+_T("  ");
				pos=m_sfName.find_last_of(_T("\\"));
				CStdString fnamewoext=m_sfName.Mid(pos+1);
				content=content+fnamewoext;
				content+=(_T("\n"));

				
				CW2A contentCstr( content.c_str() ,CP_UTF8); 
				fprintf(fp,"%s",contentCstr);
				fclose(fp);
			}
			...
			if(zlibBytesIn && zlibBytesOut){
				...
				CString str=_T("226 Transfer OK");
				str+=_T(" ");
				str+=result;
				Send(str);
			}
