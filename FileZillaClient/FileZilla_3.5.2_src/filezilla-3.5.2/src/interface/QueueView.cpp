...
void CQueueView::ProcessReply(t_EngineData* pEngineData, COperationNotification* pNotification)
{
	...
	case t_EngineData::transfer:
		...
		{
			wxString localFile=pEngineData->pItem->GetLocalFile();
			wxString remoteFile=pEngineData->pItem->GetRemoteFile();
			CLocalPath localPath=pEngineData->pItem->GetLocalPath();
			CServerPath remotePath=pEngineData->pItem->GetRemotePath();
			CServer server=pEngineData->lastServer;
			wxLongLong size=pEngineData->pItem->GetSize();
			if(pEngineData->pItem->Download()){
				// added by Pu Zhang
				wxString fileName=localPath.GetPath();
				fileName.Append(localFile);
				std::string str_sig=MDFile((wchar_t*)fileName.c_str());
				wxString sig=wxString(str_sig.c_str(),wxConvUTF8);
				ResetEngine(*pEngineData, success);
				if(!sig.IsSameAs(pEngineData->pEngine->m_sig)){
					CLogmsgNotification *notification = new CLogmsgNotification;
					notification->msgType = Error;
					notification->msg = _T("Fail! The local signature does not match the remote signature.Restarting...");
					m_pMainFrame->GetStatusView()->AddToLog(notification);
					delete notification;
					if(wxFileExists(fileName))
						wxRemoveFile(fileName);
					QueueFile(false,true,localFile,
					...
					CLogmsgNotification *notification = new CLogmsgNotification;
					notification->msgType = Status;
					notification->msg = _T("Succeed! The local signature matches the remote signature.");
					m_pMainFrame->GetStatusView()->AddToLog(notification);
					delete notification;
					//write the signature into file.
					wxString sigFileName=fileName;
					int pos=sigFileName.find_last_of(_T("."));
					sigFileName=sigFileName.Left(pos);
					sigFileName.Append(_T(".md5"));
					FILE *fp=_wfopen(sigFileName,L"w");
					wxString content=sig.Upper();
					content.Append(_T("  "));
					content.Append(localFile);
					content.Append(_T("\n"));
					fprintf(fp,"%s",content.mb_str(wxConvUTF8));
					fclose(fp);

				}
			}else{
				bool storsuccess=pEngineData->pEngine->m_storsuccess;
				// added by Pu Zhang
				wxString fileName=localPath.GetPath();
				fileName.Append(localFile);
				ResetEngine(*pEngineData, success);
				if(!storsuccess){
					...
					m_pMainFrame->GetStatusView()->AddToLog(notification);
					delete notification;
					QueueFile(false,false,localFile,
						remoteFile,localPath,
						remotePath,server,
						size);
					QueueFile_Finish(true);
