...
void CFtpControlSocket::ParseResponse()
{
	...
	if(GetCurrentCommandId()==cmd_transfer||GetCurrentCommandId()==cmd_rawtransfer){
		if(m_Response.Left(23)==_T("150 Connection accepted")){
			int pos=m_Response.find_last_of(_T(" "));
			m_pEngine->m_sig=m_Response.Mid(pos+1);
		}else if(m_Response.Left(3)==_T("226")){
			int pos=m_Response.find_last_of(_T(" "));
			int len=m_Response.Length();
			wxString result=m_Response.Mid(pos+1);
			if(result==_T("YES")){
				m_pEngine->m_storsuccess=true;
				m_Response=m_Response.Left(pos);
			}else if(result==_T("NO")){
				m_pEngine->m_storsuccess=false;
				m_Response=m_Response.Left(pos);
			}
		}
	}
...
int CFtpControlSocket::FileTransferSend()
{
	...
	case filetransfer_transfer:
		...
		else if (pData->resume)
		{
			...
			if (!pData->download){
				cmd += _T(" ");
				std::string str_hash=MDFile((wchar_t*)pData->localFile.c_str());
				wxString hash=wxString(str_hash.c_str(),wxConvUTF8);
				cmd+=hash;
			}
		}
		...
		if (pData->localFileSize >= ((wxFileOffset)1 << (i ? 31 : 32)))
		{
			...
			case unknown:
				...
				else if (pData->remoteFileSize > pData->localFileSize)
				{
					...
					wxString cmd=_T("RETR ") + pData->remotePath.FormatFilename(pData->remoteFile, !pData->tryAbsolutePath);
					return Transfer(cmd, pData);
				}
				break;
			case no:
